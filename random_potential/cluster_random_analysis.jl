...
max_clim = tryparse(Float64, ARGS[2])
...
function plot_2D_density(freq_array, title, filename)
    ...
    clim = (0,1)
    freq_plot = heatmap(x_freq, y_freq, freq_array', dpi=150, clim=clim, title=title)
...
const n = tryparse(Int64,simulation_args[1])
const domain = tryparse(Float64,simulation_args[2])
const rho = tryparse(Float64,simulation_args[3])
const fourier_interval = tryparse(Int64,simulation_args[4])
const seed = tryparse(Int64,simulation_args[5])
const time_max = tryparse(Float64, simulation_args[6])
const time_interval = tryparse(Float64, simulation_args[7])
const A = tryparse(Float64,simulation_args[8])
const D2 = round(tryparse(Float64,simulation_args[9]),digits=3)
...
const run_name_1 = simulation_args[13]
const run_name_2 = simulation_args[14]
...
normalized_hist = average_hist/max_clim
brighter_normalized_hist = normalized_hist * 1
# plot_1D_density(average_hist, "x", L"D2=%$(D2)", "1D_density_x_D2=$(D2)", focus_idx, ylims=(0,2000))
# plot_1D_density(average_hist, "y", L"D2=%$(D2)", "1D_density_y_D2=$(D2)", focus_idx, ylims=(0,2000))
plot_2D_density(brighter_normalized_hist, L"D_2=%$(D2)", "2D_density_D2=$(D2)")