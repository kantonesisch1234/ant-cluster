...
function ode_solver_part(n, speed, tspan_part, init_pos, init_direction; random_direction=true, sim="fixed_speed", save_plot=false, eq="pheromone", domain=domain, time_interval=time_interval, tspan=tspan)
    ...
    for (theta,arr) in enumerate(ant_coor_x)
        if isempty(arr)
            push!(ant_coor_x_end, nothing)
        else
            push!(ant_coor_x_end, ant_coor_x[theta][end])
        end
    end
    
    for (theta,arr) in enumerate(ant_coor_y)
        if isempty(arr)
            push!(ant_coor_y_end, nothing)
        else
            push!(ant_coor_y_end, ant_coor_y[theta][end])
        end
    end
    
     for (theta,arr) in enumerate(ant_direction)
        if isempty(arr)
            push!(end_direction, nothing)
        else
            push!(end_direction, ant_direction[theta][end])
        end
    end   
    
# 	ant_coor_x_end = [ant_coor_x[theta][-1] for theta in 1:n]
# 	ant_coor_y_end = [ant_coor_y[theta][-1] for theta in 1:n]
	end_pos = [ant_coor_x_end, ant_coor_y_end]
# 	end_direction = [ant_direction[theta][-1] for theta in 1:n]
	
	return (get_freq_array(ant_coor_x, ant_coor_y), (end_pos, end_direction))
...
function ode_solver(n, speed, chunks; random_direction=true, init_pos=(0,0), init_direction=nothing, sim="fixed_speed", save_plot=false, eq="pheromone", domain=domain, time_interval=time_interval, tspan=tspan)
	...
	tspan_part = (tspan[2]-tspan[1])/chunks

	init_pos, init_direction = set_initial_conditions(n, speed; random_direction=true, init_pos=init_pos, init_direction=init_direction, sim=sim, save_plot=save_plot, eq=eq)
	
	
	for loop in 1:chunks
		arr, end_condition = ode_solver_part(n, speed, tspan_part, init_pos, init_direction; random_direction=true, sim="fixed_speed", save_plot=false, eq="pheromone")
		hist = hist + arr
		init_pos = end_condition[1]
		init_direction = end_condition[2]
    end	  
    return hist
end
